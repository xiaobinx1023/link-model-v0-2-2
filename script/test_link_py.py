...
def main() -> None:
    ...
    if link.aggressor_ports:
        # Pick the strongest aggressor (largest NEXT/FEXT peak) to avoid plotting flat zero-response ports.
        aggr_rank = []
        for p in link.aggressor_ports:
            pr = link.get_aggressor_victim_pulse_response(p, include_total=False)
            peak_next = float(np.max(np.abs(pr["next_impulse"])))
            peak_fext = float(np.max(np.abs(pr["fext_impulse"])))
            peak = max(peak_next, peak_fext)
            aggr_rank.append((p, peak, peak_next, peak_fext))
        aggr_rank.sort(key=lambda x: x[1], reverse=True)
