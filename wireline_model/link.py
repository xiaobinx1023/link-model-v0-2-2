...
class Link:
    ...
    def __init__(
        ...
        self._dominant_aggressors: list[tuple[int, float, float, float]] = []
    ...
    def _update_xtalk_impulses(
        ...
        for port in self.aggressor_ports:
            ...
            peak_next = float(np.max(np.abs(tf_next))) if tf_next.size else 0.0
            peak_fext = float(np.max(np.abs(tf_fext))) if tf_fext.size else 0.0
            peak = peak_next if peak_next >= peak_fext else peak_fext
            self._dominant_aggressors.append((int(port), peak, peak_next, peak_fext))
    ...
    def find_dominant_aggressors(self) -> list[tuple[int, float, float, float]]:
        """Rank aggressors by coupled peak (descending) using current .sNp + link termination setup."""
        if len(self.aggressor_ports) == 0:
            return []
        if len(self._dominant_aggressors) != len(self.aggressor_ports):
            self.update_chan_afe_impulses()
        return list(self._dominant_aggressors)
